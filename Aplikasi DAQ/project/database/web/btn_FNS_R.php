...
if (isset($_GET['seq4-status'])) {
    ...
    if ($loadCurrentPart > 0) {
        ...
        $query4 = "SELECT * FROM part_production WHERE part_name = '$partName' AND position = 'Right' AND seq3_status = 'OK' AND qrcode IS NOT NULL AND status = 'waiting'";
        $result4 = $mysqli->query($query4);
        $counting_R = mysqli_num_rows($result4);
        $result4->close();
        ...
        if ($counting_R > 0) {
            ...
            if ($cek_part > 0) {
                ...
                $query5 = "UPDATE remaining_part SET qty = qty + $counting_R, date_time = '$datetime' WHERE part_name = '$partName'";
                $mysqli->query($query5);
            } else {
                $query5 = "INSERT INTO remaining_part (part_name, pn_api, pn_cust, qty) VALUES ('$partName','$pnApi', '$pnCust', '$counting_R')";
                $mysqli->query($query5);
            }
