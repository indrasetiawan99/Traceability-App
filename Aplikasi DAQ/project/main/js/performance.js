$(document).ready(function () {
  ...
  setInterval(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        br1_from.setHours(
          Number(br1_f.substring(0, 2)),
          Number(br1_f.substring(3, 5)),
          Number(br1_f.substring(6, 8))
        );
        var br1_to = new Date();
        br1_to.setHours(
          Number(br1_t.substring(0, 2)),
          Number(br1_t.substring(3, 5)),
          Number(br1_t.substring(6, 8))
        );
        var br2_from = new Date();
        br2_from.setHours(
          Number(br2_f.substring(0, 2)),
          Number(br2_f.substring(3, 5)),
          Number(br2_f.substring(6, 8))
        );
        var br2_to = new Date();
        br2_to.setHours(
          Number(br2_t.substring(0, 2)),
          Number(br2_t.substring(3, 5)),
          Number(br2_t.substring(6, 8))
        );
        var br3_from = new Date();
        br3_from.setHours(
          Number(br3_f.substring(0, 2)),
          Number(br3_f.substring(3, 5)),
          Number(br3_f.substring(6, 8))
        );
        var br3_to = new Date(dt_now);
        br3_to.setHours(
          Number(br3_t.substring(0, 2)),
          Number(br3_t.substring(3, 5)),
          Number(br3_t.substring(6, 8))
        );
        ...
        if (dt_created > dt_start) {
          if (shift == "shift1") {
            if (dt_created >= br3_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 4200000) / t_prod
              );
            } else if (dt_created >= br3_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br3_from) - 3600000) /
                  t_prod
              );
            } else if (dt_created >= br2_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 3600000) / t_prod
              );
            } else if (dt_created >= br2_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br2_from) - 600000) /
                  t_prod
              );
            } else if (dt_created >= br1_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 600000) / t_prod
              );
            } else if (dt_created >= br1_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br1_from)) / t_prod
              );
            } else if (dt_created < br1_from) {
            ...
            if (dt_created >= br3_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 6600000) / t_prod
              );
            } else if (dt_created >= br3_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br3_from) - 5400000) /
                  t_prod
              );
            } else if (dt_created >= br2_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 5400000) / t_prod
              );
            } else if (dt_created >= br2_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br2_from) - 3000000) /
                  t_prod
              );
            } else if (dt_created >= br1_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 3000000) / t_prod
              );
            } else if (dt_created >= br1_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br1_from)) / t_prod
            ...
            if (dt_created >= br3_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 7200000) / t_prod
              );
            } else if (dt_created >= br3_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br3_from) - 6000000) /
                t_prod
              );
            } else if (dt_created >= br2_to) {
              accumulation = Math.round(
                (gt_created - gt_start - 6000000) / t_prod
              );
            } else if (dt_created >= br2_from) {
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - gt_br2_from) - 3600000) /
                  t_prod
              );
              ...
              accumulation = Math.round(
                (gt_created - gt_start - (gt_created - br_OT_from) - 3000000) /
                  t_prod
              );
            } else if (dt_created >= br1_to) {
              accumulation = Math.round(
        ...
        if (shift != "shift3_2") {
          if (
            (dt_now >= br1_from && dt_now <= br1_to) ||
            (dt_now >= br2_from && dt_now <= br2_to) ||
            (dt_now >= br3_from && dt_now <= br3_to)
          ) {
            action = false;
          } else {
            action = true;
          ...
          if (
            ...
            (dt_now >= br2_from && dt_now <= br2_to) ||
            (dt_now >= br3_from && dt_now <= br3_to) ||
            (dt_now >= br_OT_from && dt_now <= br_OT_to)
          ) {
            action = false;
          } else {
            action = true;
            ...
            if (sec_start > 59) {
              ...
              if (min_start > 59) {
                ...
                if (hour_start > 23) {
                  ...
                  if (
                    month[dt_now.getMonth()] == "January" ||
                    month[dt_now.getMonth()] == "March" ||
                    month[dt_now.getMonth()] == "May" ||
                    month[dt_now.getMonth()] == "July" ||
                    month[dt_now.getMonth()] == "August" ||
                    month[dt_now.getMonth()] == "October" ||
                    month[dt_now.getMonth()] == "December"
                  ) {
                    if (date_start > 31) {
                      date_start -= 31;
                      month_start += 1;
                  ...
                  if (
                    month[dt_now.getMonth()] == "April" ||
                    month[dt_now.getMonth()] == "June" ||
                    month[dt_now.getMonth()] == "September" ||
                    month[dt_now.getMonth()] == "November"
                  ) {
                    if (date_start > 30) {
                      date_start -= 30;
                      month_start += 1;
            ...
            if (t_after - t_before >= t_prod) {
              ...
              if(counter <= target){
                $.ajax({
                  type: "POST",
                  url:
                  "http://10.14.134.44/project/database/web/qualityDowntime.php",
  ...
  setInterval(function () {
    $.ajax({
      type: "POST",
      url: "http://10.14.134.44/project/database/web/update_quality.php",
      data: "action=Update",
      success: function (data) {},
    });
  }, 1000); // update about every second
  ...
  setInterval(function () {
    $.ajax({
      type: "POST",
      url: "http://10.14.134.44/project/database/web/auto_finish.php",
      data: "action=read",
      success: function (data) {},
    });
  }, 1000); // update about every second
  ...
  setInterval(function() {
    $.ajax({
      type: "POST",
      url: "http://10.14.134.44/project/database/web/oee_counting.php",
      data: "action=update",
      success: function (data) {},
    });
  }, 1000);
